...
@export_tool_button( "Update" ) var update_node_acion = update_node
...
func _process(_delta: float) -> void:
	...
	var current_size = SaveManager.discovered_areas.size()
	if current_size != _last_check_size:
		_last_check_size = current_size
		update_visibility()
	
	# Atualiza PlayerIndicator quando pausado
	if get_tree().paused and visible:
		if SceneManager.current_scene_uid == linked_scene:
			display_player_location()

func update_visibility() -> void:
	if SaveManager.is_discovered_area(linked_scene):
		visible = true
	else:
		visible = false
...
func display_player_location() -> void:
	...
	var indicator: Control = get_node("%PlayerIndicator")
	if not indicator:
		return
		
	
	var local_player_pos: Vector2 = (player.global_position - indicator_offset) / SCALE_FACTOR
	var indicator_pos: Vector2 = local_player_pos - (indicator.size / 2.0)
	
	var clamp_margin: Vector2 = Vector2(3, 0)
	
	indicator_pos.x = clampf(
		indicator_pos.x,
		clamp_margin.x,
		size.x - clamp_margin.x - indicator.size.x
	)
	
	indicator_pos.y = clampf(
		indicator_pos.y,
		0,
		max(0, size.y - indicator.size.y)
	)
	
	indicator.position = self.position + indicator_pos
