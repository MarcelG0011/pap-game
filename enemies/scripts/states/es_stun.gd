...
@export var knockback_strenght : float = 100
var vel_x : float = 0
...
func start() -> void:
	...
	timer = 0
	_calc_velovity( blackboard.damage_source )
...
func _calc_velovity( a : AttackArea ) -> void:
	vel_x = 1
	if a.global_position.x > enemy.global_position.x:
		vel_x = -1
	vel_x *= knockback_strenght
	pass
